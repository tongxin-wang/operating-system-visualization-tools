...
namespace ProcesserScheduler
{
    ...
    public partial class ProcessorMainForm : Form
    {
        ...
        public List<Processer> processerList = new List<Processer>();
        int count = 0;
        MainWindow mw;
        public ProcessorMainForm(MainWindow mw)
        {
            InitializeComponent();
            this.mw = mw;
            //初始化控件
        ...
        {
            Processer A = new Processer(lab_A.Text, (int)(ucTrackBarApro.Value), (int)(ucTrackBarArunTime.Value));
            Processer B = new Processer(lab_B.Text, (int)(ucTrackBarBpro.Value), (int)(ucTrackBarBrunTime.Value));
            Processer C = new Processer(lab_C.Text, (int)(ucTrackBarCpro.Value), (int)(ucTrackBarCrunTime.Value));
            Processer D = new Processer(lab_D.Text, (int)(ucTrackBarDpro.Value), (int)(ucTrackBarDrunTime.Value));

            if (count == 0)
            {
                //装入list中
                this.processerList.Add(A);
                this.processerList.Add(B);
                this.processerList.Add(C);
                this.processerList.Add(D);

                ProcessorWorkForm form2 = new ProcessorWorkForm(processerList,this);
